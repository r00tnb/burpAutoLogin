...
public final class AutoLogin extends JSplitPane implements ProxyHandler, IContextMenuFactory {
    ...
    private AutoLogin() {
                ...
                if (e.getButton() == MouseEvent.BUTTON1 && table.getSelectedRowCount()>0) {
                    ...
                    if(table.getSelectedRowCount() > 0){
                        ...
                        deleteItem.addActionListener(new ActionListener() {
                            ...
                            public void actionPerformed(ActionEvent e) {
                                // TODO Auto-generated method stub
                                if(JOptionPane.showConfirmDialog(AutoLogin.this, Util.l("are you sure?it will remove all info of the item")) != 0) return;
        ...
        if (table.getRowCount() == 0) {
            // 添加默认自动登录
            try {
                JsonArray itemArray = (JsonArray) new JSONParser().fromJSON(Util.getStringFromFile("/burp/autologin/resources/default_config.json"));
    ...
    public List<JMenuItem> createMenuItems(IContextMenuInvocation invocation) {
        ...
        try {
            if (invocation.getToolFlag() == IBurpExtenderCallbacks.TOOL_REPEATER
                ...
                JMenuItem updateRequestMenu = new JMenuItem(Util.l("update current request"));
                JMenuItem generateTokenReplaceMenu = new JMenuItem(Util.l("generating a token replace rule by selection text"));
                List<JMenuItem> result = new Vector<>();
                IHttpRequestResponse requestResponse = invocation.getSelectedMessages()[0];
                Message msg = new Message(requestResponse);
                AutoLoginItem currentItem = null;
                ...
                if(menu.isEnabled()){
                    updateRequestMenu.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent e) {
                            // TODO Auto-generated method stub
                            showReLoginDialog(requestResponse);
                        }
                    ...
                    if(invocation.getSelectionBounds() == null){
                        ...
                        Session session = currentItem.getSession();
                        int[] bounds = invocation.getSelectionBounds();
                        Pattern pattern = Util.getPatternFromSelectedBounds(new String(requestResponse.getRequest()), bounds[0], bounds[1]);
                        if(pattern == null){
                            generateTokenReplaceMenu.setEnabled(false);
                        }else{
                            generateTokenReplaceMenu.addActionListener(new ActionListener(){
    ...
    private void showAddTokenReplaceDialog(Session session, Pattern pattern){
        ...
        for (Iterator<Message> iterator = session.getLoginMessages().iterator(); iterator
                .hasNext();) {
        ...
        addButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ee) {
                ...
                try {
                    ...
                    session.getTokenReplaceModel().append(token,Pattern.compile(pattern),
     ...
     */
    private void showReLoginDialog(IHttpRequestResponse requestResponse) {
        JDialog dialog = new JDialog();
        ...
        Message msg = new Message(requestResponse);
        for (Iterator<AutoLoginItem> iterator = table.getItemList().iterator(); iterator.hasNext();) {
            AutoLoginItem item = iterator.next();
            Session session = item.getSession();
            if (item.getDomain().equals(msg.getDomain())) {
